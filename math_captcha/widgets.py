...
class MathCaptchaWidget(forms.MultiWidget):
    template_name = "simple_math_captcha/captcha.html"
    ...
    def __init__(
        ...
        self.question_tmpl = question_tmpl or _(
            "What is %(num1)i %(operator)s %(num2)i? (Just a human check, we get a lot of spam!)"
        )
        self.question_html = None
        widget_attrs = {"placeholder": "Answer"}
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        return super(MathCaptchaWidget, self).render(
    ...
    def verify_numbers(self, start_int, end_int):
        ...
        if start_int < 0 or end_int < 0:
            raise Warning(
                "MathCaptchaWidget requires positive integers "
                "for start_int and end_int."
            )
        elif end_int < start_int:
            raise Warning(
                "MathCaptchaWidget requires end_int be greater " "than start_int."
            )
